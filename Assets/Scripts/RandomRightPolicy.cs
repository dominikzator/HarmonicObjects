...
public class RandomRightPolicy : AnimationPropagationPolicy
{
    ...
    public override IEnumerable<IEnumerable<GridElement>> GetNext(AnimationComponent animationComponent)
    {
        ...
        transitions.Add(MoveDirection.UP, new List<Vector2> { Vector2.right});
        transitions.Add(MoveDirection.DOWN, new List<Vector2> { Vector2.right});
        ...
        while (true)
        {
            ...
            else
            {
                ...
                float turnProbability = (currentDirection == MoveDirection.RIGHT) ? GlobalReferencesHolder.TurnProbability : 1f - GlobalReferencesHolder.TurnProbability;
